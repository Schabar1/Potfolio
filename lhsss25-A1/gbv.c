...
int
find_document (const Library *lib, const char *name)
{
...
int
gbv_create (const char *filename)
{
...
int
gbv_open (Library *lib, const char *filename)
{
...
int
gbv_add (Library *lib, const char *archive, const char *docname)
{
...
int
gbv_remove (Library *lib, const char *docname)
{
  ...
  for (int i = 0; i < lib->count; i++)
    {
      ...
      printf ("%s %ld %s %ld\n", lib->docs[i].name, lib->docs[i].size,
              date_str, lib->docs[i].offset);
    }
...
int
cmp_date (const void *a, const void *b)
{
  const Document *d1 = a;
  const Document *d2 = b;
...
int
gbv_order (Library *lib, const char *archive, const char *criteria)
{
...
int
gbv_view (const Library *lib, const char *docname)
{
  int idx = find_document (lib, docname);
