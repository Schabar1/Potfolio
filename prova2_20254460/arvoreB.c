...
bool
removerChaveArvoreB (struct arvoreB *arvore, int32_t chave)
{
  if (arvore == NULL || arvore->raiz == NULL)
    return false;

  bool removido = removerChaveNodo (arvore, arvore->raiz, chave);

  // Se a raiz ficou sem chaves e não é folha, a árvore diminui
  if (arvore->raiz->n == 0 && !arvore->raiz->folha)
    {
      struct nodo *novaRaiz = arvore->raiz->filhos[0];
      free (arvore->raiz->chaves);
